...
module.exports = {
  ...
  addtoCart: (id, user) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (userCart) {
        let proIndex = userCart.products.findIndex(
        ...
        if (proIndex != -1) {
          db.get()
            .collection(collection.CART_COLLECTIONS)
            .updateOne(
              { user: user, "products.item": ObjectId(id) },
              {
  ...
  cancelOrder: (id) => {
    return new Promise((resolve, reject) => {
      ...
      db.get()
        ...
        .updateOne({ _id: ObjectId(id) }, { $set: { OrderStatus: true , status:"  -  ",cancelStatus:true} });
      resolve();
    });
  },
