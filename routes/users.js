...
paypal.configure({
  ...
  'client_id': 'AeCcPJjFlQVn6PxXdat0I1xT3KokM9arU0-u_RnGCNJzJBLfknaW5yGXEFWI0nZMtYJmJ6W3BPgpgSCw',
  'client_secret': 'EO4uxMoFVqgfiRgxmtUYa4u0P9M5ORHt03JTHaVy8YTvYJ8iLGKhqgEQ8FWa0nO4kV3oTsWGb5_IbhJe'
});
...
router.post('/order', verifyLogin, async (req, res) => {
  ...
  if (req.body['paymentMethod'] === "COD") {
    userHelpers.placeOrder(userId, req.body, products, req.session.total).then((orderId) => {
      // let user = req.session.user
      req.session.orderId = orderId
      res.json({ CODsuccess: true })
    })
    //  res.redirect('/users/orderBook')
  } else if (req.body['paymentMethod'] === "razorpay") { //razorpay execution
    console.log("..............................total");
    console.log(req.session.total);
    userHelpers.placeOrder(userId, req.body, products, req.session.total)
    userHelpers.generateRazorpay(req.session.orderId, req.session.total).then((response) => {
...
router.get('/orderbook', verifyLogin, async (req, res) => {
  let userId = req.session.user._id
  let user = req.session.user
  let orderDetails = await db.get().collection(collection.ORDER_COLLECTIONS).find({ user: userId }).sort({ time: -1 }).toArray()
...
router.post('/updateuserprofile', verifyLogin, userController.updateUserDetails)
